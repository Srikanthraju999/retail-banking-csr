...
function getCaseTypeId(ct: PegaCaseType): string {
  return ct.ID ?? ct.caseTypeID ?? '';
}
...
export function CasesPage() {
  ...
  async function handleCreate(ct: PegaCaseType) {
    const caseTypeID = getCaseTypeId(ct);
    ...
    try {
      ...
      // Response is nested: data.caseInfo.assignments[0].ID
      const caseInfo = (created.data as Record<string, unknown>)?.caseInfo as Record<string, unknown> | undefined;
      const assignments = caseInfo?.assignments as Array<Record<string, unknown>> | undefined;
      const assignmentId = assignments?.[0]?.ID as string | undefined;
      ...
      if (assignmentId) {
        ...
        const caseId = (caseInfo?.ID as string) ?? (created.ID as string) ?? '';
        if (caseId) {
          navigate(`/cases/${encodeURIComponent(caseId)}`, { state: created });
        }
      }
    } catch (err) {
